...
export async function POST(req: Request) {
  ...
  if (file.size > MAX_FILE_BYTES) return NextResponse.json({ error: `حجم الملف يتجاوز ${fileSize(MAX_FILE_BYTES)}` }, { status: 413 });
  ...
  // يُمرَّر الملفُ نفسه — `File` هو `Blob` — فلا تُصنع نسخةٌ ثانية منه في الذاكرة
